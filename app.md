...
| `POST` | `/api/settings/{name}` | Body `{value}` (str/int/float/bool). Sets one setting. **404** if `name` is not one of the widgets `/api/settings` lists — the write path is scoped to the read path's allowlist, so action widgets (shutter, focus drives), status widgets and read-only widgets are unreachable here. 400 with the underlying error on any other failure. On success re-reads and returns the full settings list so the UI can pick up any settings that changed as a side effect (e.g. aperture limits shifting with ISO). |
